...
int thrusterMinValue = 0; // Sets minimum throttle level for ESC (0 - 180)
int thrusterMaxValue = 180; // Sets maximum throttle level for ESC (0 - 180)
int verticalThrusterMin = 7; // Sets minimum throttle level for vertical ESC (0 - 180)
int verticalThrusterMax = 179; // Sets maximum throttle level for vertical ESC (0 - 180)
...
void calculateThrusters() {
  ...
  if (cbut == 1 && zbut == 0) {
    ...
    pos3 = verticalThrusterMin - 90; // Not full 90 to try and resolve the no down thruster on full rate setting
  } else {
    pos3 = 0;
  }
